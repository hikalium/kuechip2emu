...
function initView() {
    ...
    initMemTable(codeMemSels, "#codememtable", codeMemChanged, 0x000);
}
...
function initCPU() {
    ...
    codeMemory = [];
    if (localStorage[LSKEY_CodeMemory] !== undefined) {
        codeMemory = JSON.parse(localStorage[LSKEY_CodeMemory]);
    }
    //
    if (localStorage[LSKEY_DataMemory] !== undefined) {
        dataMemory = JSON.parse(localStorage[LSKEY_DataMemory]);
    }
    //
    for (var i = 0; i < 0x100; i++) {
